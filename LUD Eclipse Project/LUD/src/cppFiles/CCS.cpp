...
bool CCS::operator==(CCS &y)
{
    ...
    if(this->numCols != y.numCols || this->numRows != y.numRows)
    {
        ...
        cout << "Numbers of rows and columns doesn't match for " << this->name << endl;
        return false;
    }
    ...
    if(this->val.size() != y.val.size())
    {
        ...
        cout << "Numbers of NNZ doesn't match for " << this->name << endl;
        cout << this->val.size() << "\n";
        cout << y.val.size() << "\n";
        return false;
    }
    ...
    if(! std::equal(this->colPtr.begin(), this->colPtr.end(), y.colPtr.begin()))
    {
        ...
        cout << "Numbers of number of elements in columns doesn't match for " << this->name << endl;
        return false;
    }
    if(! std::equal(this->rowInd.begin(), this->rowInd.end(), y.rowInd.begin()))
    {
        ...
        cout << "Numbers of row iondices don't match for " << this->name << endl;
        return false;
    }

    //Getting the column index for debugging purpose
    /*int col_ind = 0;
    int aa,bb;
    int iterations = this->colPtr.size();
    vector<int> column_index;
    while(col_ind<iterations){
       aa = this->colPtr[col_ind];
       bb = this->colPtr[col_ind+1];
       for(int ii = 0;ii<(bb-aa);ii++){
           column_index.push_back(col_ind);
       }
       //cout<<column_index[0]<<", "<<column_index[1]<<", "<<column_index[2]<<", "<<column_index[3]<<", "<<column_index[4]<<", "<<column_index[5]<<", "<<column_index[6]<<endl;
       col_ind++;
    }*/
    ...
    for(i = 0; i < this->val.size(); i++)
    {
        if(abs(this->val[i] - y.val[i]) > abs(g_errThreshold * y.val[i]))
        {
            ...
            //cout << "Values don't match for " <<i<<", "<< "("<<rowInd[i] <<","<<column_index[i]<<")"<<" , "<<" this->val[i] = "<<this->val[i]<<" y.val[i] = "<<y.val[i]<<" difference = "<<abs(this->val[i] - y.val[i])<<endl;
            return false;
        }
    }
...
{
    this->BRAMAddr.assign(this->rowInd.size(), BRAMAddrNULL); //BRAMAddrNULL is defined in stdafx.h
    this->BRAMInd.assign(this->rowInd.size(), BRAMIndNULL); //BRAMIndNULL is defined in stdafx.h
}
...
void CCS::generateBRAMCoeFiles(const vector<string> bramNames)
{
    ...
    //Debug
    /*cout << "File CCS.cpp, Function generateBRAMCoeFiles\n";
    cout<<"BRAMAddr.........................................\n";
    for(int i=0;i<this->BRAMAddr.size();i++){
	cout<<i<<": "<<BRAMAddr[i]<<std::endl;
    }

    cout<<"BRAMInd.........................................\n";
    for(int i=0;i<this->BRAMInd.size();i++){
	cout<<i<<": "<<BRAMInd[i]<<std::endl;
    }*/
    cout<<"Maxmemaddress = "<<maxMemAddress<<"* 4"<<"\n";
    ...
    for(int i = 0; i < bramNames.size(); i++)
    {
	    ...
	    //cout<<"bramData["<<i<<"]["<<j<<"]"<<bramData[i][j]<<"\n";
